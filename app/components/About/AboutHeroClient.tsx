...
export default function AboutHeroClient({
  ...
  return (
    <section
      className="animate-fade-in relative min-h-[580px] overflow-hidden sm:min-h-[620px] lg:min-h-[680px]"
    >
      {/* Background Images */}
      ...
      <div className="relative z-10 flex min-h-[580px] items-center justify-center px-5 py-16 text-center sm:min-h-[620px] sm:px-8 sm:py-0 lg:min-h-[680px] lg:px-10">
        <div className="mx-auto max-w-[800px]">
          ...
          {paragraphs.map((paragraph, index) => (
            <p
              key={index}
              className={`mx-auto max-w-[700px] text-sm leading-7 text-white/90 md:text-[15px] ${
                index === 0 ? "mt-6" : "mt-4"
              }`}
            >
              {paragraph}
